...
describe(generatePodmiotUpowazniony.name, () => {
  ...
  it('adds Rola when RolaPU has value', () => {
    ...
    expect(translateMap).toHaveBeenCalledWith(baseData.RolaPU, TRolaPodmiotuUpowaznionegoFA2);
    expect(
      result.some((r) =>
        (r as any).text?.includes(
          'Rola: Organ egzekucyjny - w przypadku, o którym mowa w art. 106c pkt 1 ustawy'
        )
      )
    ).toBe(true);
  });
