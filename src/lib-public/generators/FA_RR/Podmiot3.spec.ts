...
import type { Podmiot3 } from '../../types/FaRR.types';
...
describe(generatePodmiot3.name, () => {
  ...
  it('generates minimal podmiot3 structure', () => {
    const podmiot: Podmiot3 = { DaneIdentyfikacyjne: {} };
  ...
  it('adds column2 when DaneKontaktowe is present', () => {
    const podmiot: Podmiot3 = {
      DaneIdentyfikacyjne: {},
      DaneKontaktowe: [{}],
    };
    ...
    expect(
      flatColumn1.some(
        (c) =>
          ...
          c.text.includes('Rola: Faktor - w przypadku gdy na fakturze występują dane faktora')
      )
    ).toBe(true);
    expect(
      flatColumn1.some((c) => typeof c.text === 'string' && c.text.includes('Opis roli: opisRoli'))
    ).toBe(true);
